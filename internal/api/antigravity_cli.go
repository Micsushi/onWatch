...
func (r *AntigravityCLIRunner) Fetch(ctx context.Context) (*AntigravitySnapshot, error) {
	...
	stop := context.AfterFunc(r.rootCtx, cancel)
	defer stop()
	defer cancel()
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *AntigravityCLIRunner) launch(binPath string) (*agySession, error) {
	...
	// Fetch forwards shutdown to readiness/HTTP work; teardown owns killing and
	// waiting for the child, including when startup fails.
	cmd := p.Command(binPath)
	// A remote-style login flow prevents an unattended collector from opening a browser.
