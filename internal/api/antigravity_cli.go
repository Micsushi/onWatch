	...
	if home, err := os.UserHomeDir(); err == nil {
		...
		if runtime.GOOS == "windows" {
			candidates = append(candidates,
				filepath.Join(home, "AppData", "Local", "agy", "bin", binName),
...
func (r *AntigravityCLIRunner) Fetch(ctx context.Context) (*AntigravitySnapshot, error) {
	...
	stopCancel := context.AfterFunc(r.rootCtx, cancel)
	defer stopCancel()
	defer cancel()
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	groups, err := parseAgyQuotaSummary(summary)
	if err != nil {
		r.recordFailureLocked()
		return nil, err
	}
...
func (r *AntigravityCLIRunner) launch(binPath string) (*agySession, error) {
	...
	// Own cancellation: go-pty v0.2.3 races its Windows context waiter with Wait.
	cmd := p.Command(binPath)
	configureAgyLaunch(cmd)
	if home, err := os.UserHomeDir(); err == nil {
		cmd.Dir = filepath.Join(home, ".onwatch", "agy-quota")
	...
	stopCancel := context.AfterFunc(r.rootCtx, func() { _ = cmd.Process.Kill() })
	go func() { defer close(done); defer stopCancel(); _ = cmd.Wait() }()
	closeLaunch, err := finishAgyLaunch(cmd)
	if err != nil {
		_ = cmd.Process.Kill()
		_ = p.Close()
		<-done
		return nil, fmt.Errorf("antigravity cli: contain quota process: %w", err)
	}
	...
	return &agySession{pty: p, cmd: cmd, closeLaunch: closeLaunch, done: done}, nil
}
		...
		if err == nil && len(ports) > 0 {
			if conn, _ := r.client.probeForConnectAPI(ctx, ports, ""); conn != nil {
				if body, status, perr := r.post(ctx, conn.BaseURL, agyQuotaSummaryRPC); perr == nil && status == http.StatusOK {
					if _, err := parseAgyQuotaSummary(body); err == nil {
						return conn, nil
					}
				}
			}
...
func parseAgyQuotaSummary(body []byte) ([]AntigravityQuotaSummaryGroup, error) {
	var response AntigravityQuotaSummaryResponse
	if err := json.Unmarshal(body, &response); err != nil {
		return nil, err
	}
	if response.Response == nil || len(response.Response.Groups) == 0 {
		return nil, ErrAntigravityInvalidResponse
	}
	NormalizeAntigravityQuotaSummary(response.Response)
	count := 0
	for _, group := range response.Response.Groups {
		count += len(group.Buckets)
	}
	if count == 0 {
		return nil, ErrAntigravityInvalidResponse
	}
...
func (r *AntigravityCLIRunner) killSession(sess *agySession) {
	...
	if sess.closeLaunch != nil {
		sess.closeLaunch()
		sess.closeLaunch = nil
	} else if sess.cmd != nil && sess.cmd.Process != nil {
	...
	if sess.done != nil {
		...
		case <-time.After(5 * time.Second):
			r.logger.Warn("agy process has not finished after termination")
		}
	}
}
