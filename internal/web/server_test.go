...
func freePort(t *testing.T) int {
	t.Helper()
	// Bind loopback only so macOS does not raise a firewall prompt.
...
func TestServer_StartsOnPort(t *testing.T) {
	...
	server := NewServer(freePort(t), handler, logger, "admin", passHash, "127.0.0.1", "", "")
...
func TestServer_RequiresCSRFHeader_OnPost(t *testing.T) {
	...
	server := NewServer(freePort(t), handler, logger, "", "", "127.0.0.1", "", "")
...
func TestServer_MetricsUsesBearerAuthWithoutDashboardRedirect(t *testing.T) {
	...
	passHash, _ := HashPassword("test")
	server := NewServer(freePort(t), handler, logger, "admin", passHash, "127.0.0.1", "", "metrics-secret")
