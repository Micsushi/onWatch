...
func (r *Runtime) Run(ctx context.Context) error {
	...
	defer lock.Close()
	if err = lockCollectorFile(lock); err != nil {
		return fmt.Errorf("collector spool is already in use: %w", err)
	}
	if err = r.spool.recoverPartialTails(); err != nil {
		return err
	}
...
func (r *Runtime) uploadOnce(ctx context.Context) error {
	...
	for i, result := range response.Results {
		...
		case "accepted", "duplicate", "enriched":
		case "rejected":
			if err := r.quarantine(records[i].Event, result); err != nil {
				return fmt.Errorf("persist rejected event: %w", err)
			}
		default:
			return fmt.Errorf("server acknowledgement status is invalid")
		}
	}
	now := time.Now().UTC()
